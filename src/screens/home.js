...
class Home extends Component {
        ...
        <div className="recipe">
          <div className="row">
            <div className="recipe-img-container hero-img col-md-8 col-xs-12">
              <a href={`/article/${RECIPES[0].slug}`}><img src={RECIPES[0].img_url} className="recipe-img" /></a>
            </div>
    ...
    return (
      <div className="hero">
        <div className="recipe">
          <div className="row">
            <div className="recipe-img-container hero-img col-md-8 col-xs-12">
              <a href="#"><img src={this.state.instagram_data.data[0].images.standard_resolution.url} className="recipe-img" /></a>
            </div>
          <div className="recipe-sub-hero col-md-4 col-xs-12">
            <div>
              <a href="#"><h1>most recent post</h1></a>
              <p className="date">{moment.unix(this.state.instagram_data.data[0].created_time).format('LL')}</p>
              <p className="description">{this.state.instagram_data.data[0].caption == null ? "" : this.state.instagram_data.data[0].caption.text}</p>
            </div>
            <div className="recipe-links">
              <div className="share">
                <a href="#"><i className="fas fa-heart"></i></a>
  ...
  render_recipes() {
    return this.state.instagram_data.data.map((post, i) => 
          <div className="recipe col-md-4 col-xs-12" key={i} style={i < 1 ? {display: "none"} : {display: "block"} }>
            <div className="recipe-img-container">
              <a href="#"><img src={post.images.standard_resolution.url} className="recipe-img" /></a>
            </div>
            <div className="recipe-sub">
              <div>
                <a href="#"><h2>post {i}</h2></a>
                <p className="date">{moment.unix(post.created_time).format('LL')}</p>
                <p className="description">{post.caption == null ? "" : post.caption.text}</p>
              </div>
              <div className="recipe-links">
                <div className="share">
                  <a href="#"><i className="fas fa-heart"></i></a>
  ...
  render_recipes_test() {
    return RECIPES.map((recipe, i) =>
      <div className="recipe col-md-4 col-xs-12" key={i} style={i < 1 ? {display: "none"} : {display: "block"} }>
        <div className="recipe-img-container">
          <a href={`/article/${recipe.slug}`}><img src={recipe.img_url} className="recipe-img" /></a>
        </div>
        <div className="recipe-sub">
          <div>
            <a href={`/article/${recipe.slug}`}><h2>{recipe.title}</h2></a>
            {/* <Link to={`/article/${recipe.slug}`}><h2>{recipe.title}</h2></Link> */}
  ...
  render() {
    return (
      <div className="home container">
        {this.state.is_loading ? null : this.render_hero_test()}
        <div className="main">
          <div className="row">
          {this.state.is_loading ? null : this.render_recipes_test()}
          </div>
        </div>
      </div>
    );
